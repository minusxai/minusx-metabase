...
export async function getUserContext(userId?: number): Promise<UserContext> {
  ...
  const [edits, creations] = await Promise.all([
    handlePromise(searchUserEdits(user.id), "Error getting user edits", []),
    handlePromise(searchUserCreations(user.id), "Error getting user creations", []),
  ]);

  const editQueries = edits;
  const creationQueries = creations;
  const allQueries = Array.from(new Set([...editQueries, ...creationQueries]));
